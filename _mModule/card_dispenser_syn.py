...
def arg_check():
    ...
    global BAUD_RATE_KYT
    global BAUD_RATE_MTK
    global DEVICE_TYPE
    ...
    for arg in sys.argv:
        if arg in ['9600', '19200', '38400']:
            ...
            if DEVICE_TYPE == 'KYT':
                BAUD_RATE_KYT = int(arg)
            if DEVICE_TYPE == 'MTK':
                BAUD_RATE_MTK = int(arg)
...
if __name__ == '__main__':
    ...
    for i in range(multiply):
        if DEVICE_TYPE == "SYN":
            response["cmd"] = 'SIMPLY_EJECT_SYN'
            simply_eject_syn(port, response)
        elif DEVICE_TYPE == "MTK":
            response["cmd"] = 'SIMPLY_EJECT_MTK'
            simply_eject_mtk(port, response)
        elif DEVICE_TYPE == "KYT":
            response["cmd"] = 'SIMPLY_EJECT_KYT'
            simply_eject_kyt(port, response)
        else:
            response["cmd"] = 'SIMPLY_EJECT'
            simply_eject(port, response)
        print(str(response))
    exit('Done', 0)
